...
print(check_output(["ls", "../test.json"]).decode("utf8"))
...
import pylab
plt.rcParams['figure.figsize'] = 10, 10 # 차트의 기본 크기 설정
#get_ipython().run_line_magic('matplotlib', 'inline')# jupyter에서만 그래프 가능
...
#Load the data.
train = pd.read_json("/Users/jaejin/dev/keras/train.json") # Data Read
...
X_band_1=np.array([np.array(band).astype(np.float32).reshape(75, 75) for band in train["band_1"]])  # np.array.astype 형변환 해줌
print('------------')
print(X_band_1.shape)
print(X_band_1[0][0][0])
print(np.size(X_band_1[0], 0))
print(np.size(X_band_1, 0))
print('------------')

#sum
sum = np.zeros((75,75) , dtype= np.float32)
avg = np.zeros((75,75)  ,dtype= np.float32)
for y in range(0,np.size(X_band_1[0][0], 0)):
    for x in range(0, np.size(X_band_1[0], 0)):
        for i in range(0, np.size(X_band_1, 0)):
            sum[y][x] = sum[y][x] + X_band_1[i][y][x]

print('------------- sum')
# avg
for y in range(0,np.size(X_band_1[0][0], 0)):
    for x in range(0, np.size(X_band_1[0], 0)):
        avg[y][x] = sum[y][x] / np.size(X_band_1, 0)

print('----------------avg')

# insert avg
X_band_avg = np.zeros((np.size(X_band_1,0), 75, 75), dtype= np.float32)
for i in range(0, np.size(X_band_1, 0)):
    for y in range(0, np.size(X_band_1[0][0], 0)):
        for x in range(0, np.size(X_band_1[0], 0)):
            X_band_avg[i][y][x] = (avg[y][x] - X_band_1[i][y][x])  # 평균에서 데이터를 뺀다

    print(X_band_avg[i])


X_band_fliplr_1 = np.zeros((np.size(X_band_1,0), 75, 75), dtype= np.float32)
#X_band_flipud_1 = np.zeros((np.size(X_band_1,0), 75, 75), dtype= np.float32)


for i in range(0,np.size(X_band_1, 0)):
    X_band_fliplr_1[i] = np.fliplr(X_band_1[i]) # 좌우 반전
    #X_band_flipud_1[i] = np.flipud(X_band_1[i]) # 상하 반전



X_band_2=np.array([np.array(band).astype(np.float32).reshape(75, 75) for band in train["band_2"]])

X_band_fliplr_2 = np.zeros((np.size(X_band_2,0), 75, 75), dtype= np.float32)
#X_band_flipud_2 = np.zeros((np.size(X_band_2,0), 75, 75), dtype= np.float32)


for i in range(0,np.size(X_band_1, 0)):
    X_band_fliplr_2[i] = np.fliplr(X_band_2[i]) # 좌우 반전
    #X_band_flipud_2[i] = np.flipud(X_band_2[i]) # 상하 반전

X_band_1 = np.concatenate((X_band_1, X_band_fliplr_1), axis = 0) # 원래 데이터에 추가하기
X_band_2 = np.concatenate((X_band_2, X_band_fliplr_2), axis = 0) # 원래 데이터에 추가하기
print(X_band_1.shape)
...
target_train = np.concatenate((target_train, target_train), axis = 0) # 원래 데이터에 추가하기
